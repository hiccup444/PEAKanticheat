...
namespace AntiCheatMod
{
    ...
    public class AntiCheatPlugin : BaseUnityPlugin, IConnectionCallbacks, IMatchmakingCallbacks, IInRoomCallbacks, IOnEventCallback
    {
        ...
        private void Update()
        {
            // Process pending item checks for spawn detection
            RPCDetection.ProcessPendingItemChecks();
            
            // Handle queued visual logs
            if (_queuedLogs.Count > 0)
            {
                var (message, onlySendOnce, sfxJoin, sfxLeave) = _queuedLogs.Dequeue();
                LogVisually(message, onlySendOnce, sfxJoin, sfxLeave, true);
            }
        }
        ...
        private void OnPlayerUnblocked(int actorNumber)
        {
            ...
            // Clear flag spawn counts for this player
        ...
        // Dev Steam ID for special join message
        private const string DEV_STEAM_ID = "STEAM_0:0:143296685";

        // Harmony patch to intercept join messages and show dev indicator
        [HarmonyPatch(typeof(PlayerConnectionLog), "OnPlayerEnteredRoom")]
        [HarmonyPrefix]
        public static bool PreOnPlayerEnteredRoom(PlayerConnectionLog __instance, Photon.Realtime.Player newPlayer)
        {
            // Skip if it's the local player or "Bing Bong" (game's special case)
            if (newPlayer.IsLocal || newPlayer.NickName == "Bing Bong")
            {
                return true; // Let the original method handle it
            }

            // Check if this is the dev joining
            bool isDev = false;
            try
            {
                ...
                if (steamId != CSteamID.Nil)
                {
                    ...
                    isDev = steamIdString == DEV_STEAM_ID;
                    
                    if (isDev)
                    {
                        Logger.LogInfo($"[DEV JOIN] Detected dev joining: {newPlayer.NickName} (Steam ID: {steamIdString})");
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.LogWarning($"[DEV JOIN] Error checking Steam ID for {newPlayer.NickName}: {ex.Message}");
            }

            // If it's the dev, show custom message
            if (isDev)
            {
                try
                {
                    // Get the color fields using reflection
                    var userColorField = __instance.GetType().GetField("userColor", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
                    var joinedColorField = __instance.GetType().GetField("joinedColor", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
                    var sfxJoinField = __instance.GetType().GetField("sfxJoin", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
                    var addMessageMethod = __instance.GetType().GetMethod("AddMessage", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
                    var getColorTagMethod = __instance.GetType().GetMethod("GetColorTag", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);

                    if (userColorField != null && joinedColorField != null && addMessageMethod != null && getColorTagMethod != null)
                    {
                        var userColor = (Color)userColorField.GetValue(__instance);
                        var joinedColor = (Color)joinedColorField.GetValue(__instance);

                        // Create custom dev message
                        string userColorTag = (string)getColorTagMethod.Invoke(__instance, new object[] { userColor });
                        string joinedColorTag = (string)getColorTagMethod.Invoke(__instance, new object[] { joinedColor });

                        string newValue = userColorTag + " [Anticheat Dev] " + newPlayer.NickName + "</color>";
                        string customMessage = joinedColorTag + LocalizedText.GetText("JOINEDTHEEXPEDITION", true).Replace("#", newValue) + "</color>";

                        // Add the custom message
                        addMessageMethod.Invoke(__instance, new object[] { customMessage });

                        // Play join sound if available
                        if (sfxJoinField != null)
                        {
                            var sfxJoin = sfxJoinField.GetValue(__instance);
                            if (sfxJoin != null)
                            {
                                var playMethod = sfxJoin.GetType().GetMethod("Play", new Type[] { typeof(Vector3) });
                                playMethod?.Invoke(sfxJoin, new object[] { Vector3.zero });
                            }
                        }

                        Logger.LogInfo($"[DEV JOIN] Custom dev join message displayed for {newPlayer.NickName}");
                        return false; // Prevent the original method from running
                    }
                }
                catch (Exception ex)
                {
                    Logger.LogError($"[DEV JOIN] Error creating custom dev message: {ex.Message}");
                }
            }

            return true; // Let the original method handle it for non-dev players
        }
        ...
        private static CSteamID GetPlayerSteamID(Photon.Realtime.Player player)
        {
            try
            {
                ...
                if (steamNameToId.ContainsKey(player.NickName))
                {
                    ...
                    // If there's only one Steam player with this name and one Photon player, it's a match
                    if (possibleSteamIDs.Count == 1 && photonPlayersWithSameName.Count == 1)
                    {
                        Logger.LogInfo($"[GetPlayerSteamID] Unique match found: {player.NickName} -> {possibleSteamIDs[0]}");
                        return possibleSteamIDs[0];
                    }

                    // If there are multiple players with the same name, we need to find the unassigned one
                    if (possibleSteamIDs.Count > 1)
                    {
                        // Find which Steam IDs are already assigned to other Photon players
                        var assignedSteamIDs = new HashSet<CSteamID>();
                        foreach (var otherPlayer in PhotonNetwork.PlayerList)
                        {
                            if (otherPlayer.ActorNumber != player.ActorNumber)
                            {
                                var otherSteamID = GetPlayerSteamID(otherPlayer);
                                if (otherSteamID != CSteamID.Nil)
                                {
                                    assignedSteamIDs.Add(otherSteamID);
                                }
                            }
                        }

                        // Find the first unassigned Steam ID
                        foreach (var steamId in possibleSteamIDs)
                        {
                            if (!assignedSteamIDs.Contains(steamId))
                            {
                                Logger.LogInfo($"[GetPlayerSteamID] Found unassigned Steam ID for {player.NickName}: {steamId}");
                                return steamId;
                            }
                        }

                        Logger.LogWarning($"[GetPlayerSteamID] No unassigned Steam IDs found for {player.NickName}");
                    }
                }

                // Fallback: try to find an unassigned Steam ID by name
                ...
                // Find which Steam IDs are already assigned to other Photon players
                var usedSteamIDs = new HashSet<CSteamID>();
                foreach (var otherPlayer in PhotonNetwork.PlayerList)
                {
                    if (otherPlayer.ActorNumber != player.ActorNumber)
                    {
            ...
            {
                Logger.LogError($"[GetPlayerSteamID] Error getting Steam ID for {player.NickName}: {ex.Message}");
                return CSteamID.Nil;
            }
        }
