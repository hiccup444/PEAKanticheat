...
namespace AntiCheatMod
{
    ...
    public static class RPCDetection
    {
        ...
        public static bool PreOwnerActorNrSetter(PhotonView __instance, int value)
        {
            ...
            // TEMPORARY: Detect item spawning by checking if player takes ownership of items they shouldn't have
            if (PhotonNetwork.IsMasterClient && value > 0 && value != PhotonNetwork.LocalPlayer.ActorNumber)
            {
                var newOwner = PhotonNetwork.CurrentRoom?.GetPlayer(value);
                if (newOwner != null)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        // Check if the player should have this item
                        bool shouldHaveItem = PlayerShouldHaveItem(value, itemName);
                        
                        // If they shouldn't have this item, schedule a delayed check
                        if (!shouldHaveItem)
                        {
                            // Schedule a delayed check instead of immediately flagging as spawn
                            ScheduleItemCheck(value, itemName, __instance.ViewID);
                        }
                    }
                }
            }
        ...
        [HarmonyPatch(typeof(PhotonNetwork), "Instantiate", new Type[] { typeof(string), typeof(Vector3), typeof(Quaternion), typeof(byte), typeof(object[]) })]
        [HarmonyPrefix]
        public static bool PrePhotonNetworkInstantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
        {
            // Allow the instantiate to proceed (logging removed)
            return true;
        }

        // Flag spawn tracking
        private static Dictionary<int, int> _flagSpawnCounts = new Dictionary<int, int>();
        private static Dictionary<int, DateTime> _flagSpawnTimestamps = new Dictionary<int, DateTime>();
        private const int MAX_FLAG_SPAWNS = 5;
        private const int FLAG_SPAWN_RESET_MINUTES = 10; // Reset count after 10 minutes

        // Item spawn tracking for delayed re-check
        private static Dictionary<int, (string itemName, int viewId, DateTime timestamp)> _pendingItemChecks = new Dictionary<int, (string, int, DateTime)>();
        private const int ITEM_CHECK_DELAY_FRAMES = 3; // Wait 3 frames before re-checking

        // Method to clear flag counts for a player (called when they're unblocked)
        public static void ClearFlagCounts(int actorNumber)
        {
            if (_flagSpawnCounts.ContainsKey(actorNumber))
            {
                _flagSpawnCounts.Remove(actorNumber);
                _flagSpawnTimestamps.Remove(actorNumber);
                AntiCheatPlugin.Logger.LogInfo($"[FLAG COUNT CLEARED] Cleared flag spawn counts for Actor #{actorNumber}");
            }
        }

        // Method to check if a player has a backpack
        public static bool PlayerHasBackpack(int actorNumber)
        {
        ...
        // Method to check if a player has a specific item in their backpack
        public static bool PlayerHasItemInBackpack(int actorNumber, string itemName)
        {
            var allPlayers = UnityEngine.Object.FindObjectsOfType<Player>();
            foreach (var player in allPlayers)
            {
                ...
                if (photonView != null && photonView.Owner != null && 
                    photonView.Owner.ActorNumber == actorNumber)
                {
                    if (player.backpackSlot.hasBackpack && player.backpackSlot.data != null)
                    {
                        // Check the backpack's stored data
                        foreach (var dataEntry in player.backpackSlot.data.data)
                        {
                            // Look for item references in the data
                            string dataString = dataEntry.Value.ToString().ToLower();
                            if (dataString.Contains(itemName.ToLower()))
                            {
                                return true;
                            }
                        }
        ...
        // Method to check if a player should have an item (consolidated logic)
        public static bool PlayerShouldHaveItem(int actorNumber, string itemName)
        {
            // Check if they have/had this item recently
            if (AntiCheatPlugin.PlayerHadItem(actorNumber, itemName, 5f))
            {
                return true;
            }
            ...
            foreach (var character in allCharacters)
            {
                var charPhotonView = character.GetComponent<PhotonView>();
                if (charPhotonView != null && charPhotonView.Owner != null &&
                    charPhotonView.Owner.ActorNumber == actorNumber)
                {
                    var characterData = character.GetComponent<CharacterData>();
                    if (characterData != null && characterData.currentItem != null &&
                        characterData.currentItem.name == itemName)
                    {
                        return true;
                    }
                }
            ...
            // Check if it's in their backpack
            if (PlayerHasItemInBackpack(actorNumber, itemName))
            {
                return true;
            }
        ...
        // Method to schedule a delayed item check
        public static void ScheduleItemCheck(int actorNumber, string itemName, int viewId)
        {
            _pendingItemChecks[actorNumber] = (itemName, viewId, DateTime.Now);
            AntiCheatPlugin.Logger.LogInfo($"[ITEM CHECK SCHEDULED] Scheduled delayed check for {itemName} (ViewID: {viewId}) for Actor #{actorNumber}");
        }

        // Method to process pending item checks (called from Update)
        public static void ProcessPendingItemChecks()
        {
            var currentTime = DateTime.Now;
            var keysToRemove = new List<int>();
            
            foreach (var kvp in _pendingItemChecks)
            {
                int actorNumber = kvp.Key;
                var (itemName, viewId, timestamp) = kvp.Value;
                
                // Check if enough time has passed (roughly 3 frames at 60fps = 50ms)
                if ((currentTime - timestamp).TotalMilliseconds >= 50)
                {
                    var player = PhotonNetwork.CurrentRoom?.GetPlayer(actorNumber);
                    if (player != null)
                    {
                        // Re-check if they should have the item
                        if (!PlayerShouldHaveItem(actorNumber, itemName))
                        {
                            // Check detection settings
                            bool shouldBlock = DetectionManager.ShouldAutoBlock(DetectionType.ItemSpawning);
                            bool shouldShowVisual = DetectionManager.ShouldShowVisualWarning(DetectionType.ItemSpawning);
                            bool shouldLog = DetectionManager.ShouldLogToConsole(DetectionType.ItemSpawning);

                            // Log to console if enabled
                            if (shouldLog)
                            {
                                AntiCheatPlugin.Logger.LogWarning($"[ITEM SPAWN CONFIRMED] {player.NickName} still doesn't have {itemName} (ViewID: {viewId}) after delay!");
                                AntiCheatPlugin.Logger.LogWarning($"[ITEM SPAWN CONFIRMED]   This confirms it was likely spawned by a cheater.");
                            }

                            // Show visual warning if enabled
                            if (shouldShowVisual)
                            {
                                AntiCheatPlugin.LogVisually($"{{userColor}}{player.NickName}</color> {{leftColor}}spawned {itemName}!</color>", false, false, true);
                            }

                            // Block player if auto-block is enabled
                            if (shouldBlock)
                            {
                                AntiCheatPlugin.BlockPlayer(player, $"Item spawning detected: {itemName}", DetectionType.ItemSpawning);
                            }

                            // Record the detection
                            DetectionManager.RecordDetection(DetectionType.ItemSpawning, player, $"Spawned item: {itemName}");
                        }
                        else
                        {
                            AntiCheatPlugin.Logger.LogInfo($"[ITEM CHECK CLEARED] {player.NickName} now has {itemName} (ViewID: {viewId}) - legitimate pickup.");
                        }
                    }
                    
                    keysToRemove.Add(actorNumber);
                }
            }
            ...
            {
                _pendingItemChecks.Remove(key);
            }
        }

        // CreatePrefabRPC patch for Constructable class
        [HarmonyPatch(typeof(Constructable), "CreatePrefabRPC")]
        [HarmonyPrefix]
        public static bool PreConstructableCreatePrefabRPC(Constructable __instance, Vector3 position, Quaternion rotation, PhotonMessageInfo info)
        {
            // Only log if we're the master client and someone else is calling this
            if (PhotonNetwork.IsMasterClient && info.Sender != null && !info.Sender.IsMasterClient)
            {
                string senderName = info.Sender.NickName ?? "Unknown";
                string prefabName = __instance.constructedPrefab?.name ?? "Unknown";
                int senderActorNumber = info.Sender.ActorNumber;
                
                // Check for Flag prefab spam
                if (prefabName.StartsWith("Flag", StringComparison.OrdinalIgnoreCase))
                {
                    // Initialize tracking if needed
                    if (!_flagSpawnCounts.ContainsKey(senderActorNumber))
                    {
                        _flagSpawnCounts[senderActorNumber] = 0;
                        _flagSpawnTimestamps[senderActorNumber] = DateTime.Now;
                    }

                    // Check if we need to reset the counter (10 minute window)
                    if ((DateTime.Now - _flagSpawnTimestamps[senderActorNumber]).TotalMinutes >= FLAG_SPAWN_RESET_MINUTES)
                    {
                        _flagSpawnCounts[senderActorNumber] = 0;
                        _flagSpawnTimestamps[senderActorNumber] = DateTime.Now;
                    }

                    // Increment flag spawn count
                    _flagSpawnCounts[senderActorNumber]++;

                    // Check if they've exceeded the limit
                    if (_flagSpawnCounts[senderActorNumber] > MAX_FLAG_SPAWNS)
                    {
                        AntiCheatPlugin.Logger.LogError($"[FLAG SPAM DETECTED] {senderName} has spawned {_flagSpawnCounts[senderActorNumber]} Flag prefabs in {FLAG_SPAWN_RESET_MINUTES} minutes!");
                        AntiCheatPlugin.LogVisually($"{{userColor}}{senderName}</color> {{leftColor}}tried to crash the lobby with prefabs!</color>", false, false, true);
                        
                        // Force block for flag spam (regardless of UI setting)
                        AntiCheatPlugin.BlockPlayer(info.Sender, $"Flag spam detected: {_flagSpawnCounts[senderActorNumber]} flags in {FLAG_SPAWN_RESET_MINUTES} minutes", DetectionType.PrefabDetection);
                        
                        // Record the detection
                        DetectionManager.RecordDetection(DetectionType.PrefabDetection, info.Sender, $"Flag spam: {_flagSpawnCounts[senderActorNumber]} flags");
                        
                        return false; // Block the RPC
                    }
                    else if (_flagSpawnCounts[senderActorNumber] == MAX_FLAG_SPAWNS)
                    {
                        // Warning at the limit
                        AntiCheatPlugin.Logger.LogWarning($"[FLAG SPAM WARNING] {senderName} has spawned {_flagSpawnCounts[senderActorNumber]}/{MAX_FLAG_SPAWNS} Flag prefabs!");
                        AntiCheatPlugin.LogVisually($"{{userColor}}{senderName}</color> {{leftColor}}reached flag spawn limit ({_flagSpawnCounts[senderActorNumber]}/{MAX_FLAG_SPAWNS})!</color>", false, false, true);
                    }
                }
                
                // Check detection settings for normal prefab detection
                bool shouldBlock = DetectionManager.ShouldAutoBlock(DetectionType.PrefabDetection);
                bool shouldShowVisual = DetectionManager.ShouldShowVisualWarning(DetectionType.PrefabDetection);
                bool shouldLog = DetectionManager.ShouldLogToConsole(DetectionType.PrefabDetection);

                // Log to console if enabled
                if (shouldLog)
                {
                    AntiCheatPlugin.Logger.LogWarning($"[HOST CREATE PREFAB RPC LOG] Player {senderName} called CreatePrefabRPC:");
                    AntiCheatPlugin.Logger.LogWarning($"[HOST CREATE PREFAB RPC LOG]   Class: Constructable");
                    AntiCheatPlugin.Logger.LogWarning($"[HOST CREATE PREFAB RPC LOG]   Method: CreatePrefabRPC");
                    AntiCheatPlugin.Logger.LogWarning($"[HOST CREATE PREFAB RPC LOG]   Prefab: {prefabName}");
                    AntiCheatPlugin.Logger.LogWarning($"[HOST CREATE PREFAB RPC LOG]   Position: {position}");
                    AntiCheatPlugin.Logger.LogWarning($"[HOST CREATE PREFAB RPC LOG]   Rotation: {rotation}");
                }

                // Show visual warning if enabled
                if (shouldShowVisual)
                {
                    AntiCheatPlugin.LogVisually($"{{userColor}}{senderName}</color> {{leftColor}}spawned {prefabName}!</color>", false, false, true);
                }

                // Block player if auto-block is enabled (for non-Flag prefabs)
                if (shouldBlock && !prefabName.StartsWith("Flag", StringComparison.OrdinalIgnoreCase))
                {
                    AntiCheatPlugin.BlockPlayer(info.Sender, $"Unauthorized prefab spawning: {prefabName}", DetectionType.PrefabDetection);
                }

                // Record the detection
                DetectionManager.RecordDetection(DetectionType.PrefabDetection, info.Sender, $"Spawned prefab: {prefabName}");
            }
            return true; // Allow the RPC to proceed
        }
    }
}